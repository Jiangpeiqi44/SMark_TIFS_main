...
class DW_Encoder(nn.Module):
    ...
    def forward(self, x, watermark):
        d0 = self.conv1(x)  # d0 16x256x256
        d1 = self.down1(d0)  # d1 32x128x128
        d2 = self.down2(d1)  # d2 64x64x64
        d3 = self.down3(d2)  # d3 128x32x32

        d4 = self.down4(d3)  # d4 256x16x16

        # print(d0.shape,d1.shape,d2.shape,d3.shape,d4.shape)
        u3 = self.up3(d4)
