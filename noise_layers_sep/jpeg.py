...
class JpegTest(nn.Module):
	def __init__(self, Q=50, subsample=2, path="temp/"):
	...
	def forward(self, image_cover_mask):
		...
		for i in range(image.shape[0]):
			...
			# os.remove(file)
			encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), int(self.Q)]
			result, encimg = cv2.imencode('.jpg', single_image, encode_param)
