...
class Coach:
    ...
    def configure_datasets(self):
        ...
        #                             img_transform=transforms.Compose(
        #                                 [transforms.Resize((1024, 1024)), TO_TENSOR, NORMALIZE]),
        #                             label_transform=transforms.Compose(
        #                                 [transforms.Resize((1024, 1024)), MASK_CONVERT_TF_DETAILED, TO_TENSOR]),  # MASK_CONVERT_TF_DETAILED
        #                             vis_transform=transforms.Compose(
        #                                 [transforms.Resize((1024, 1024)), TO_TENSOR]),
        #                             fraction=self.opts.ds_frac,
        #                             flip_p=self.opts.flip_p)
    ...
    def train(self):
        ...
        while self.global_step <= self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):                    
                ...
                if self.opts.train_D and (self.global_step % self.opts.d_every == 0):
                    ...
                    with autocast():
                        ...
                        if self.opts.d_reg_every!=-1 and batch_idx % self.opts.d_reg_every==0:
                            ...
                            # self.optimizer_D.step()
                            scaler.scale(self.opts.r1_lambda / 2 * r1_loss * self.opts.d_reg_every + 0 * real_pred[0]).backward()
                            scaler.step(self.optimizer_D)
                            scaler.update()
                ...
                # Logging related
                if self.rank==0 and (self.global_step % self.opts.image_interval == 0 or (self.global_step < 100 and self.global_step % 25 == 0)):
