...
from network.WM_Enc import WMEnc
...
class AADGenerator(nn.Module):
    def __init__(
        ...
        self.message_length_robust = message_length_robust
        ...
        self.WMEnc_1 = WMEnc(message_length, blocks, channels, attention)
        self.WMEnc_2 = WMEnc(message_length_robust, blocks, channels, attention)
        self.gamma = torch.nn.Parameter(torch.rand(8))
        # self.beta = torch.nn.Parameter(torch.rand(8))
    ...
    def forward(self, inputs):
        ...
        StyleGAN inversion: 14 x 512
        """
        z_att, z_id, watermark, watermark_robust = inputs
        z_att_wm1 = self.WMEnc_1(z_att, watermark)
        z_att_wm2 = self.WMEnc_2(z_att, watermark_robust)
        z_att = [
            z_att_wm1[i] * torch.clamp(self.gamma[i],0,1) + z_att_wm2[i] * torch.clamp(1-self.gamma[i],0,1)
        ...
        m = F.interpolate(
            self.AADResBlk1(m, z_att[0], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk2(m, z_att[1], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk3(m, z_att[2], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk4(m, z_att[3], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk5(m, z_att[4], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk6(m, z_att[5], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
        ...
        m = F.interpolate(
            self.AADResBlk7(m, z_att[6], z_id),
            scale_factor=2,
            mode="bilinear",
            align_corners=True,
        )
