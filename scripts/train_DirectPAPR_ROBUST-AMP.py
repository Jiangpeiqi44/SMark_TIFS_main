...
from common import l2_norm,alignment,visualize_train_results,visualize_train_results_multi,visualize_train_results_multi_e4s,generate_seqstate,visualize_train_results_multi_lite,visualize_train_results_multi_lite2
import datetime
import torchvision.transforms as transforms
from e4s_AMP.src.datasets.dataset import CelebAHQDataset,get_transforms,TO_TENSOR,NORMALIZE,MASK_CONVERT_TF,FFHQDataset,FFHQ_MASK_CONVERT_TF,MASK_CONVERT_TF_DETAILED,FFHQ_MASK_CONVERT_TF_DETAILED
from e4s_AMP.src.utils import torch_utils
from e4s_AMP.src.models.networks import Net3
from e4s_AMP.src.models.stylegan2.model import Generator, Discriminator
from e4s_AMP.src.training.ranger import Ranger
from e4s_AMP.src.criteria.style_loss import StyleLoss
from e4s_AMP.src.criteria.adv_loss import AdvDLoss, AdvGLoss, DR1Loss, GPathRegularizer
from e4s_AMP.src.criteria.lpips.lpips import LPIPS
from e4s_AMP.src.criteria.face_parsing.face_parsing_loss import FaceParsingLoss
from e4s_AMP.src.criteria.id_loss import IDLoss
from e4s_AMP.src.criteria.w_norm import WNormLoss
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if self.e4s_config.train_D and (
                ...
                if (
                    ...
                    # self.optimizer_D.step()
                    self.optimizer_D.zero_grad()
                    self.scaler.scale(self.e4s_config.r1_lambda
                        / 2
                        * r1_loss
                        * self.e4s_config.d_reg_every
        ...
        for epoch in range(self.opts.max_epoch):
            ...
            if self.opts.local_rank == 0:
                ...
                if (
                    ...
                    and epoch >= 3*self.opts.max_epoch // 4
                ):  # 过半后再保存
...
def main():
    ...
    if opts.local_rank == 0:
        ...
        shutil.copyfile("./scripts/train_DirectPAPR_ROBUST-AMP.py", os.path.join(output_dir,"Train_screenshot.py")) 
        # logging.basicConfig(
        #     level=logging.INFO,
        #     format="%(message)s",
        #     handlers=[
