...
sys.path.append(".")
sys.path.append("..")
from e4s.src.datasets.utils import make_dataset
    ...
    def load_single_image(self, index):
        ...
        if self.load_vis_img:
            ...
            if self.vis_transform is not None:
                label_vis = self.vis_transform(label_vis)
        else:
...
class FFHQDataset(Dataset):
    def __init__(self, dataset_root,
        ...
        with open(osp.join(self.root, "ffhq_list.txt"),"r") as f:
            f_lines = f.readlines()
...
class CelebAHQDataset_wBaseName(Dataset):
    def __init__(self, dataset_root, mode="test",
                 img_transform=TO_TENSOR, label_transform=TO_TENSOR,
				 vis_transform=TO_TENSOR,
                 load_vis_img=False,fraction=1.0,
    ...
    def load_single_image(self, index):
        ...
        if self.load_vis_img:
            ...
            label_vis = -1  # unified interface
        return img, label, label_vis, base_name

    def __getitem__(self, idx):
        index = self.indices[idx]

        img, label, label_vis, base_name = self.load_single_image(index)
...
class FFHQDataset_wBaseName(Dataset):
    def __init__(self, dataset_root,
                 img_transform=TO_TENSOR, label_transform=TO_TENSOR,
                 fraction=1.0,
                 load_raw_label=False,
    ...
    def load_single_image(self, index):
        ...
        if self.load_raw_label:
            return img, original_label, label, label_vis, base_name
        else:
            return img, label, label_vis, base_name
