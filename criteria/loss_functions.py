...
class GANLoss(nn.Module):
    ...
    def __call__(self, input, target_is_real):
        if isinstance(input, list):
            loss = 0
            for pred_i in input:        # shape: list[3] list[1] [B,1,H,W]
                if isinstance(pred_i, list):
                    pred_i = pred_i[-1]     # shape after: [B,1,H,W]
...
class MsgLoss(nn.Module):
    def __init__(self, msg_weight, loss_mode):
        super(MsgLoss, self).__init__()
        ...
        if loss_mode == 'MSE':
            ...
            raise ValueError('Unexpected Loss Mode {}'.format(loss_mode))
    def forward(self, msg_input, msg_output):
        if self.mode == 'MSE':
            msg_loss = self.criterion(msg_input, msg_output)
            #msg_loss *= 0.5
        elif self.mode == 'MAE':
            msg_loss = self.criterion(msg_input, msg_output)
        elif self.mode == 'BCE':
            ...
            msg_loss = torch.mean(1 - torch.cosine_similarity(msg_input, msg_output, dim=1))
        return msg_loss * self.msg_weight
...
class RecLoss(nn.Module):
    def __init__(self, rec_weight, loss_mode, device):
        ...
        if self.mode == 'l2':
            ...
            self.criterion = LPIPS(device=device, net_type='alex').to(device).eval()
        elif self.mode == 'both':
            self.criterion1 = nn.MSELoss().to(device)
            self.criterion2 = LPIPS(device=device, net_type='alex').to(device).eval()
        else:
            raise ValueError('Unexpected Loss Mode {}'.format(loss_mode))            
    def forward(self, img_input, img_output):
        if self.mode == 'l2':
            rec_loss = 0.5 * self.criterion(img_input.half(), img_output.half())
        elif self.mode == 'lpips':
            rec_loss = self.criterion(img_input, img_output)
        elif self.mode == 'both':
            rec_loss = 0.5 * self.criterion1(img_input.half(), img_output.half()) + self.criterion2(img_input, img_output)
